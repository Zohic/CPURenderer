...
float sdCircle(in vec2 p, in float r)
{
	...
	if (p.x < 0.0 || p.y < 0.0)
		return sdBox(p - r / 2.0, vec2(r / 2.0, r / 2.0));

	if (p.x > r || p.y > r)
	{
		if (p.x > p.y)
		{
			return length(p - vec2(r, 0));
		}
		else
			return length(p - vec2(0, r));

	}



	float sc = max(r - p.x, r - p.y);
	float bc = min(r - p.x, r - p.y);

	float c = max(r * r - bc * bc, 0.0);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	col = mix(col, vec3(1.0, 0.0, 0.0), 1.0 - smoothstep(0.0, 0.01, abs(d)));

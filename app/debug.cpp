...
private:
	OLCDrawing() = delete;
	OLCDrawing(const OLCDrawing& other) = delete;
	OLCDrawing(OLCDrawing&& old) = delete;
	OLCDrawing& operator=(const OLCDrawing& other) = delete;
	OLCDrawing& operator=(OLCDrawing&& old) = delete;
...
public:
	...
	void FillTriangle(uint32_t x1, uint32_t y1, uint32_t x2, uint32_t y2, uint32_t x3, uint32_t y3, uint8_t r, uint8_t g, uint8_t b) const override{
...
template<typename LambdaType, typename DesiredType>
auto LambdaToFunc(LambdaType l) {
	return reinterpret_cast<DesiredType>(reinterpret_cast<void*>(l));
}
...
class MainC: public olc::PixelGameEngine {
	...
	CustomizableRasterizer raster = CustomizableRasterizer(&vertexBuffer, &drawer);
...
public:
	...
	bool OnUserCreate() override {
		...
		try {
			...
			//	heavy.glb
			//	katana.glb
			Mesh& testMesh = storage.GetMesh("triag");
			meshLoader.LoadMesh("../../../../app/meshes/scena.glb", testMesh);
			
			//testMesh.PrintData();
		
			//assert(storage.GetMesh("triag"))
			...
			transformer.SetCulling(Transformer<VertexBuffer>::CullType::NO);
		...
		try {
			...
			ri->transform.pos = Vec3(0, -9, -65.0f);
	...
	bool OnUserUpdate(float dt) override {
		...
		//f1.rot.y() += 3.14f * 5.13f * dt;
		ri->transform.rot.y() += 3.14f * 12.33f * dt;
		ri->transform.rot.z() += 3.14f * 12.33f * dt;

		if (GetKey(olc::SPACE).bPressed)
			transformer.SetCulling((Transformer<VertexBuffer>::CullType)((++cltmp)%3));

		this->DrawString(20, 20, std::to_string(vertexBuffer.GetVertexCount()) + " vertices" );
		this->DrawString(20, 60, std::to_string(cltmp % 3) + " : cull type");
